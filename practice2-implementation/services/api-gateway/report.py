...
BASE_URL = "http://localhost:8000"  # API Gateway đang chạy local
...
def get_report():
    try:
        ...
        if resp.status_code != 200:
            return {"error": "Cannot connect to API Gateway"}
        urls = resp.json()
        total_urls = len(urls)
        total_checks = 0
        for url in urls:
...
def export_csv():
    ...
    try:
        ...
        return FileResponse(
            ...
            content=content,
            media_type="text/csv",
            filename=f"uptime_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
    except Exception as e:
        return {"error": str(e)}
...
def history_csv(url: str = Query(...)):
    ...
    try:
        ...
        return FileResponse(
            ...
            content=content,
            media_type="text/csv",
            filename=f"history_{domain}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
    except Exception as e:
        return {"error": str(e)}